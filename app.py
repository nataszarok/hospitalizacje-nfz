...
from dashboard.data.loaders import (
    ...
    aggregated, admission_comparison,
)
...
try:
    ...
    with tab_mortality:
        render_mortality(result=result,result_all=result_all,min_hosp=state.min_hosp,state=state,refs=refs,product_meta=product_meta,product_legend=legend)
    with tab_admissions:
