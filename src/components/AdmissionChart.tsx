...
import type { AdmissionRow, GeographyMode, RegionOption } from "@/lib/types";

export function AdmissionChart({ rows, geographyMode, highlightedRegions, highlightedCities, regions }: {
  rows: AdmissionRow[];
  ...
  useEffect(() => {
    ...
    const render = async () => {
      ...
      const makeTrace = (subset: AdmissionRow[], name: string, color: string, emphasized = false) => ({
        ...
        mode: "markers",
        name,
        x: subset.map((row) => row.plannedAdmissions),
        y: subset.map((row) => row.urgentAdmissions),
        customdata: subset.map((row) => [row.providerName, row.nip, row.owNfz, regionByCode.get(row.owNfz) ?? row.voivodeship, row.city, row.plannedAdmissions, row.urgentAdmissions, row.totalAdmissions]),
        marker: {
      ...
      await Plotly.react(chartRef.current, traces, {
        ...
        xaxis: {
          title: { text: "Przyjęcia planowane (kod 6)", standoff: 18, font: { size: 15, color: "#344054" } },
          gridcolor: "#EEF1F5",
          zerolinecolor: "#EEF1F5",
          zerolinewidth: 3,
        },
        yaxis: {
          title: { text: "Przyjęcia nagłe (kody 2 + 3)", standoff: 18, font: { size: 15, color: "#344054" } },
  ...
  }, [rows, geographyMode, highlightedRegions, highlightedCities, regionByCode]);
